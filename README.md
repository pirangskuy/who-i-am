<!-- =========================
  PROFILE README - KURNIAWAN
  Ganti: pirangskuy
========================= -->

# Halo 👋, saya **Kurniawan Alhamdani Pandayu Putra**
🎓 **Sarjana Teknik Informatika** • 💻 Web & App Developer • 🌱 Suka membangun sistem yang rapi, cepat, dan mudah dipakai

---

## 👋 Selamat Datang di GitHub Saya!
Terima kasih sudah mampir!  
Di sini saya menyimpan proyek-proyek pengembangan aplikasi (web & mobile), eksperimen, dan dokumentasi.  
Silakan cek repositori saya—kalau ada ide kolaborasi atau masukan, saya sangat terbuka 🤝

---

## 🧑‍💻 Tentang Saya
- Fokus: **Web Development** (PHP/MySQL) & **UI/UX yang enak dipakai**
- Sering bikin: dashboard admin, sistem sekolah, CMS, fitur CRUD kompleks, laporan, dan integrasi API
- Prinsip: **clean code**, struktur rapi, dan performa

---

## 🧰 Bahasa & Teknologi yang Saya Gunakan
**Bahasa Pemrograman:**
- PHP, JavaScript, HTML, CSS
- SQL (MySQL)
- Java (Android)

**Framework / Library:**
- Laravel (opsional kalau kamu pakai), Bootstrap, React (opsional)
- Leaflet / Mapbox (peta)

**Tools:**
- Git & GitHub, Laragon/XAMPP, VS Code, Figma (opsional)

---

## 📌 Highlight Proyek (contoh, silakan sesuaikan)
- **Aplikasi Raport Digital** (PHP + MySQL) — manajemen nilai, wali kelas, rekap, dll.
- **CMS / Website Publikasi** — modul berita, artikel, toolkit, report.
- **PWA / Map-based App** — fitur peta, marker, dan tampilan interaktif.

> Tips: “Pin” 4–6 repo terbaik kamu di profil GitHub biar tampil di atas.

---

## 📊 Statistik GitHub Saya
> Ganti `pirangskuy` dengan username GitHub kamu

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&hide_border=true)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_USERNAME&layout=compact&hide_border=true)

---

## 📈 Grafik Aktivitas Proyek
![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=YOUR_GITHUB_USERNAME&hide_border=true)

---

## 🔥 Streak (opsional)
![GitHub Streak](https://streak-stats.demolab.com?user=YOUR_GITHUB_USERNAME&hide_border=true)

---

## 🏷️ Badge Skill (opsional, biar keren)
![PHP](https://img.shields.io/badge/PHP-777BB4?logo=php&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?logo=mysql&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=000)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?logo=css3&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?logo=bootstrap&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white)

---

## 📫 Kontak (isi yang kamu punya)
- Email: kiapndy@gmail.com
- LinkedIn: https://www.linkedin.com/in/kurniawanalhamdanipandayuputra
- Portfolio: https://jasaalgojo.rf.gd/

---

⭐ Kalau kamu suka dengan proyek saya, boleh kasih **star** ya!
